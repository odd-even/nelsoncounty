#!/usr/bin/env python3
"""
Full rewrite - Batch 1 (first 10 listings) - FIXED
- Extract complete sentences only
- Keep business-relevant content
...
def is_sentence_relevant(sentence: str, listing: dict) -> bool:
    ...
    generic_patterns = [
        ...
        r'james loving that same year',  # Incomplete sentence start
        r'^james loving',  # Starts with incomplete
    ]
...
def rewrite_accordion(title: str, original_content: str, listing: dict) -> str:
    """Rewrite accordion - extract only complete, relevant sentences"""
    ...
    # Extract only relevant, complete sentences
    relevant = []
    for sent in sentences:
        ...
        # Skip incomplete fragments (starts with capital letter + space + lowercase = fragment)
...
def process_listing(listing: dict, original_listing: dict) -> dict:
    ...
    for orig_title, orig_content in original_accordions.items():
        # Skip duplicate titles (keep first one)
        if orig_title in seen_titles:
            continue
        seen_titles.add(orig_title)
        
        rewritten = rewrite_accordion(orig_title, orig_content, listing)
