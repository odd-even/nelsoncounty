...
/**
 * Get (or lazily create) the secret used to sign session tokens.
 * Stored in Script Properties so it never appears in source code. If it does
 * not exist yet it is generated once on first use. Rotating this value (delete
 * the property) invalidates all outstanding sessions.
 */
function getSessionSigningSecret() {
  ...
  if (!secret) {
    ...
    Logger.log('🔐 Generated new SESSION_SIGNING_SECRET');
  }
  return secret;
}

/**
 * Compute the web-safe base64 HMAC-SHA256 signature of a payload string.
 */
function signSessionPayload_(payloadStr) {
  const sigBytes = Utilities.computeHmacSha256Signature(payloadStr, getSessionSigningSecret());
 ...
 *
 * Format: base64url(payloadJson) + "." + base64url(HMAC_SHA256(payloadJson)).
 * Because the signature requires the server-side secret, clients cannot forge
 * a token for an arbitrary email. (The previous format was unsigned base64
 * JSON, which anyone could fabricate.)
 */
function generateSessionToken(email) {
  ...
  return payloadB64 + '.' + signSessionPayload_(payloadStr);
}

/**
 * Validate session token server-side.
 *
 * Verifies the HMAC signature FIRST (so forged/tampered tokens are rejected),
 * then checks expiry and that the email is still authorized.
 */
function validateSessionToken(token) {
  try {
    ...
    // Recover the payload and verify the signature before trusting anything.
    const payloadStr = Utilities.newBlob(Utilities.base64DecodeWebSafe(parts[0])).getDataAsString();
    const expectedSig = signSessionPayload_(payloadStr);
    if (!constantTimeEquals_(parts[1], expectedSig)) {
      Logger.log('❌ validateSessionToken: Signature mismatch (forged or tampered token)');
      return { valid: false, error: 'Invalid session token' };
    }
...
/**
 * Require a valid session token, or throw. Use this to gate any action that
 * mutates data or returns sensitive material. Returns the authorized email.
 */
function requireValidSessionTokenOrThrow(sessionToken) {
  if (!sessionToken) {
    throw new Error('Unauthorized: Missing session token');
  }
  const validation = validateSessionToken(sessionToken);
  if (!validation.valid) {
    throw new Error('Unauthorized: ' + (validation.error || 'Invalid or expired session token'));
  }
...
function doGet(e) {
  try {
    ...
    if (e && e.parameter && e.parameter.action === 'getAuthorizedEmails') {
      try {
        // Require a valid session: the authorized-email list must not be public.
        requireValidSessionTokenOrThrow(e.parameter.token || e.parameter.sessionToken);
    ...
    if (e && e.parameter && e.parameter.action === 'updateAuthorizedEmails') {
      ...
      try {
        ...
        // SECURITY: require a valid signed session token. The previous
        // "email overlap" fallback allowed changing the authorized list with
        // NO token as long as the submitted list contained a known-authorized
        // email (which were public) — an account-takeover path. Removed.
        const requesterEmail = requireValidSessionTokenOrThrow(sessionToken);
    ...
    if (e && e.parameter && e.parameter.action === 'getImageKitUploadParams') {
      try {
        // Require a valid session: signed upload params must not be public,
        // otherwise anyone could upload arbitrary files to the ImageKit account.
        requireValidSessionTokenOrThrow(e.parameter.token || e.parameter.sessionToken);
        return ContentService
          .createTextOutput(JSON.stringify({
            success: true,
    ...
    if (e && e.parameter && e.parameter.action === 'generateImageDescription') {
      // Require a valid session: this calls a paid AI API.
      try {
        requireValidSessionTokenOrThrow(e.parameter.token || e.parameter.sessionToken);
      } catch (authError) {
        return ContentService
          .createTextOutput(JSON.stringify({ success: false, error: authError.toString() }))
          .setMimeType(ContentService.MimeType.JSON);
      }
    ...
    if (e && e.parameter && e.parameter.action === 'updateImageKitMetadata') {
      // Require a valid session (prevents public metadata edits).
      try {
        requireValidSessionTokenOrThrow(e.parameter.token || e.parameter.sessionToken);
      } catch (authError) {
        return ContentService
    ...
    if (e && e.parameter && e.parameter.action === 'deleteListing') {
      // Require a valid session before allowing a delete.
      try {
        requireValidSessionTokenOrThrow(e.parameter.token || e.parameter.sessionToken);
      } catch (authError) {
        return ContentService
...
function doPost(e) {
  try {
    ...
    // SECURITY: every mutating or sensitive action requires a valid signed
    // session token. Read-only auth actions (sendOTP/verifyOTP/validateSession)
    // are intentionally excluded so users can log in.
    const PROTECTED_ACTIONS = [
      'saveListing', 'replaceAllListings', 'deleteListing', 'saveCategories',
      'updateAuthorizedEmails', 'updateImageKitMetadata',
      'getImageKitUploadParams', 'generateImageDescription'
    ];
    if (PROTECTED_ACTIONS.indexOf(action) !== -1) {
      requireValidSessionTokenOrThrow(data.token || data.sessionToken || data.session);
    }
  ...
  try {
    ...
    const listings = rows.map((row, index) => {
      ...
      headers.forEach((header, colIndex) => {
        ...
        } else if (headerLower === 'isevent' || headerLower === 'is event') {
          const eventVal = String(value || '').toLowerCase();
          listing.isEvent = eventVal === 'true' || eventVal === 'yes' || eventVal === '1';
        } else if (headerLower === 'eventallday' || headerLower === 'event all day' || headerLower === 'allday' || headerLower === 'all day') {
          const allDayVal = String(value || '').toLowerCase();
          listing.eventAllDay = allDayVal === 'true' || allDayVal === 'yes' || allDayVal === '1';
        } else if (['eventstartdate', 'event start date', 'startdate', 'start date'].includes(headerLower)) {
          listing.eventStartDate = String(value || '');
        } else if (['eventenddate', 'event end date', 'enddate', 'end date'].includes(headerLower)) {
          listing.eventEndDate = String(value || '');
        } else if (['eventstarttime', 'event start time', 'starttime', 'start time'].includes(headerLower)) {
          listing.eventStartTime = String(value || '');
        } else if (['eventendtime', 'event end time', 'endtime', 'end time'].includes(headerLower)) {
          listing.eventEndTime = String(value || '');
        } else if (['eventticketurl', 'event ticket url', 'ticketurl', 'ticket url', 'rsvpurl', 'rsvp url'].includes(headerLower)) {
          listing.eventTicketUrl = String(value || '');
        } else if (['eventcost', 'event cost', 'cost', 'price'].includes(headerLower)) {
          listing.eventCost = String(value || '');
        } else if (['eventvenuename', 'event venue name', 'venuename', 'venue name', 'venue'].includes(headerLower)) {
          listing.eventVenueName = String(value || '');
  ...
  try {
    ...
    headers.forEach((header, colIndex) => {
      ...
      if (headerLower === 'slug') {
        ...
        rowData.push(listing.isEvent ? 'TRUE' : 'FALSE');
      } else if (headerLower === 'eventallday' || headerLower === 'event all day') {
        rowData.push(listing.eventAllDay ? 'TRUE' : 'FALSE');
      } else if (['eventstartdate', 'event start date'].includes(headerLower)) {
        const incomingValue = listing.eventStartDate || '';
        rowData.push(incomingValue && String(incomingValue).trim() ? parseDateString(String(incomingValue)) : '');
      } else if (['eventenddate', 'event end date'].includes(headerLower)) {
        const incomingValue = listing.eventEndDate || '';
        rowData.push(incomingValue && String(incomingValue).trim() ? parseDateString(String(incomingValue)) : '');
      } else if (['eventstarttime', 'event start time'].includes(headerLower)) {
        rowData.push(listing.eventStartTime || '');
      } else if (['eventendtime', 'event end time'].includes(headerLower)) {
        rowData.push(listing.eventEndTime || '');
      } else if (['eventticketurl', 'event ticket url'].includes(headerLower)) {
        rowData.push(listing.eventTicketUrl || '');
      } else if (['eventcost', 'event cost'].includes(headerLower)) {
        rowData.push(listing.eventCost || '');
      } else if (['eventvenuename', 'event venue name'].includes(headerLower)) {
        rowData.push(listing.eventVenueName || '');
      } else if (['customhtml', 'custom html', 'customhtml'].includes(headerLower)) {
