...
// =================================
        ...
        // Store OTP data (in production, this should be server-side)
        let otpStorage = {
            codes: {},           // email -> { code, expires, attempts }
            requestHistory: {}   // email -> [timestamps of requests]
        };
        ...
        function checkRateLimit(email) {
            const now = Date.now();
            const history = otpStorage.requestHistory[email] || [];
            ...
            // Update history
            otpStorage.requestHistory[email] = recentRequests;
        ...
        async function sendOTP(email) {
            try {
                ...
                // Store OTP (in production, this should be server-side)
                otpStorage.codes[email.toLowerCase()] = {
                    code: otpCode,
                    expires: expiresAt,
                    attempts: 0
                };
                ...
                // Record this request
                if (!otpStorage.requestHistory[email.toLowerCase()]) {
                    otpStorage.requestHistory[email.toLowerCase()] = [];
                }
                ...
                if (result.success) {
                    ...
                    delete otpStorage.codes[email.toLowerCase()];
                    return {
                        success: false,
        ...
        function verifyOTP(email, code) {
            ...
            const stored = otpStorage.codes[emailLower];
            ...
            if (Date.now() > stored.expires) {
                delete otpStorage.codes[emailLower];
                return {
                    success: false,
