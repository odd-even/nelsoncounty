...
// =================================
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Quill editors load lazily when the listing modal opens
        });
        ...
        function initializeQuillEditor() {
            if (typeof Quill === 'undefined') {
                ensureQuillLoaded().then(initializeQuillEditor);
                return;
            }
        ...
        function initializeAccordionPanel1Editor() {
            if (typeof Quill === 'undefined') {
                ensureQuillLoaded().then(initializeAccordionPanel1Editor);
                return;
            }
        ...
        function initializeAccordionPanel2Editor() {
            if (typeof Quill === 'undefined') {
                ensureQuillLoaded().then(initializeAccordionPanel2Editor);
                return;
            }
        ...
        function initializeAccordionPanel3Editor() {
            if (typeof Quill === 'undefined') {
                ensureQuillLoaded().then(initializeAccordionPanel3Editor);
                return;
            }
        ...
        function initializeAccordionPanel4Editor() {
            if (typeof Quill === 'undefined') {
                ensureQuillLoaded().then(initializeAccordionPanel4Editor);
                return;
            }
...
function updateTabsStickyStackStuck() {
    ...
    stack.classList.toggle('is-stuck', stuck);
        ...
        function sanitizeListingsFromRemote(rawListings) {
            return rawListings.map(function(listing) {
        ...
        function syncCategoriesFromListings(listings) {
            ...
            categoriesFromData.forEach(function(categoryKey) {
                ...
                } else if (TYPE_CATEGORIES[categoryKey] && categoriesFromStorage && categoriesFromStorage[categoryKey]) {
                    const storageCategory = categoriesFromStorage[categoryKey];
                    if (storageCategory.emoji) TYPE_CATEGORIES[categoryKey].emoji = storageCategory.emoji;
                    if (storageCategory.name) TYPE_CATEGORIES[categoryKey].name = storageCategory.name;
                    if (storageCategory.description) TYPE_CATEGORIES[categoryKey].description = storageCategory.description;
                    if (storageCategory.icon) TYPE_CATEGORIES[categoryKey].icon = storageCategory.icon;
                }
            });
        ...
        function finalizeLoadedListings(listings, sheetHeaders, statusMessage, logSource) {
            if (typeof data === 'undefined' || !data) {
            ...
            syncCategoriesFromListings(listings);
            refreshListingsSearchCache(listings);
            console.log('✅ Loaded ' + listings.length + ' listings from ' + logSource);
            updateSyncStatus(true, statusMessage || ('Loaded ' + listings.length + ' listings'));
            renderListings();
            populateAdminFilters();
            updateStats();
        }

        async function loadFromListingsJSON() {
            const cacheBustUrl = LISTINGS_JSON_URL + '?t=' + Date.now();
            console.log('📦 Loading listings from static JSON:', cacheBustUrl);
            const response = await fetch(cacheBustUrl, { method: 'GET', credentials: 'omit' });
            if (!response.ok) {
                throw new Error('listings.json fetch failed: ' + response.status);
            }
            const payload = await response.json();
            const rows = Array.isArray(payload) ? payload : (payload && payload.rows) || [];
            if (!rows.length) {
                throw new Error('listings.json contained no rows');
            }
            const listings = sanitizeListingsFromRemote(
                rows.map(function(row) { return mapCSVRowToListing(row); }).filter(function(l) { return l.name; })
            );
            finalizeLoadedListings(
                listings,
                payload.headers,
                'Loaded ' + listings.length + ' listings',
                'listings.json (generated ' + ((payload && payload.generatedAt) || 'unknown') + ')'
            );
            return true;
        }

        // Load listings: static JSON first (fast), then Apps Script, then CSV
        async function loadDataFromGoogleSheets(options) {
            ...
            if (!options.forceRemote && !isFileProtocol && LISTINGS_JSON_URL) {
                try {
                    if (await loadFromListingsJSON()) return;
                } catch (jsonError) {
                    console.warn('⚠️ listings.json unavailable, trying Google Sheets:', jsonError.message);
                }
            }
            
            // Try Apps Script (live data from Google Sheets)
            if (GOOGLE_APPS_SCRIPT_URL && !GOOGLE_APPS_SCRIPT_URL.includes('YOUR_SCRIPT_ID')) {
                try {
                    ...
                    if (result.success && result.listings && result.listings.length > 0) {
                        ...
                        // CRITICAL: Preserve ALL categories from localStorage, not just ones found in listings
                        // This ensures custom categories added in admin are never lost
                        if (categoriesFromStorage) {
                            Object.keys(categoriesFromStorage).forEach(function(categoryKey) {
                                if (!TYPE_CATEGORIES[categoryKey]) {
                                    // This is a custom category from admin that's not in Google Sheets yet
                                    TYPE_CATEGORIES[categoryKey] = JSON.parse(JSON.stringify(categoriesFromStorage[categoryKey]));
                        ...
                        // Save updated TYPE_CATEGORIES to localStorage to ensure persistence
                        saveCategoriesToStorage(TYPE_CATEGORIES);
        ...
        window.reloadFromSheets = async function reloadFromSheets() {
            ...
            await loadDataFromGoogleSheets({ forceRemote: true });
        }
        ...
        function refreshListingCardInGrid(listing, oldSlug) {
            const grid = document.getElementById('listingsGrid');
            if (!grid || isListingModalOpen()) {
                pendingListingsGridRefresh = true;
                return;
            }
            const slug = listing && listing.slug;
            if (!slug) {
                renderListings(lastAdminFilteredListings || data.listings, { force: true });
                return;
            }
            ...
            const existing = grid.querySelector('.flip-card[data-slug="' + CSS.escape(slug) + '"]');
            const newCard = buildListingCardElement(listing);
            if (existing && newCard) {
        ...
        function reconcileListingsGridAfterSave(listing, oldSlug, isUpdate) {
            updateListingSearchCache(listing);
            if (!isUpdate || (oldSlug && oldSlug !== listing.slug)) {
                renderListings(lastAdminFilteredListings || data.listings, { force: true });
                return;
            }
            refreshListingCardInGrid(listing, oldSlug);
        }
        ...
        let amenitiesCheckboxSignature = '';
        let filterListingsDebounceTimer = null;

        function isListingModalOpen() {
        ...
        function updateListingSearchCache(listing) {
            if (!listing) return;
            listing._searchText = buildListingSearchText(listing);
        }

        function refreshListingsSearchCache(listings) {
        ...
        function panelHasAccordionContent(title, content) {
            ...
            if (raw.includes('<') && raw.includes('>')) {
                ...
                if (text) return true;
                return !!tmp.querySelector('img, video, iframe, embed, object, a');
            }
            return true;
        }

        function setQuillContentFromValue(quillInstance, value) {
            ...
            quillInstance.root.innerHTML = lines.length
                ? lines.map(function(line) { return '<p>' + line + '</p>'; }).join('')
                : '';
        }

        function ensureAccordionPanelEditor(panelNum) {
            const initMap = {
                1: initializeAccordionPanel1Editor,
                2: initializeAccordionPanel2Editor,
                3: initializeAccordionPanel3Editor,
                4: initializeAccordionPanel4Editor
            };
            const initFn = initMap[panelNum];
            if (!initFn) return;
            initFn();
            const textarea = document.getElementById('listingAccordionPanel' + panelNum + 'Content');
            const quillMap = {
                1: function() { return quillAccordionPanel1; },
                2: function() { return quillAccordionPanel2; },
                3: function() { return quillAccordionPanel3; },
                4: function() { return quillAccordionPanel4; }
            };
            const quill = quillMap[panelNum]();
            if (textarea && quill && textarea.value && !quill.root.textContent.trim()) {
                setQuillContentFromValue(quill, textarea.value);
            }
        }

        function setupAccordionLazyFocusHandlers() {
            if (window.__accordionLazyFocusReady) return;
            window.__accordionLazyFocusReady = true;
            [1, 2, 3, 4].forEach(function(panelNum) {
                const title = document.getElementById('listingAccordionPanel' + panelNum + 'Title');
                const editor = document.getElementById('listingAccordionPanel' + panelNum + 'ContentEditor');
                const handler = function() { ensureAccordionPanelEditor(panelNum); };
                if (title) title.addEventListener('focus', handler);
                if (editor) editor.addEventListener('mousedown', handler);
            });
        }

        function initAccordionEditorsForListing(listing) {
            setupAccordionLazyFocusHandlers();
            const panels = [
                { num: 1, title: listing.accordionPanel1Title, content: listing.accordionPanel1Content },
                { num: 2, title: listing.accordionPanel2Title, content: listing.accordionPanel2Content },
                { num: 3, title: listing.accordionPanel3Title, content: listing.accordionPanel3Content },
                { num: 4, title: listing.accordionPanel4Title, content: listing.accordionPanel4Content }
            ];
            panels.forEach(function(panel) {
                const textarea = document.getElementById('listingAccordionPanel' + panel.num + 'Content');
                if (textarea) textarea.value = panel.content || '';
                if (!panelHasAccordionContent(panel.title, panel.content)) return;
                ensureAccordionPanelEditor(panel.num);
                const quillMap = { 1: quillAccordionPanel1, 2: quillAccordionPanel2, 3: quillAccordionPanel3, 4: quillAccordionPanel4 };
                setQuillContentFromValue(quillMap[panel.num], panel.content || '');
            });
        }

        function syncAmenitiesFromListing(listing) {
            renderAmenitiesCheckboxes(false);
            const checkboxes = document.querySelectorAll('#amenitiesCheckboxes input[type="checkbox"]');
            const selected = (listing && listing.amenities) || [];
            checkboxes.forEach(function(checkbox) {
                checkbox.checked = selected.indexOf(checkbox.value) > -1;
            });
        }

        window.filterListingsDebounced = function filterListingsDebounced() {
            if (filterListingsDebounceTimer) clearTimeout(filterListingsDebounceTimer);
            filterListingsDebounceTimer = setTimeout(filterListings, 220);
        };
        ...
        function filterListings() {
            ...
            const filtered = data.listings.filter(function(listing) {
                ...
                if (!listing._searchText) updateListingSearchCache(listing);
                const matchesSearch = !searchTerm || listing._searchText.indexOf(searchTerm) > -1;
            ...
            lastAdminFilteredListings = filtered;
        ...
        function renderAmenitiesCheckboxes(forceRebuild) {
            ...
            const signature = amenities.join('\0');
            if (!forceRebuild && signature === amenitiesCheckboxSignature && container.children.length > 0) {
                return;
            }
            amenitiesCheckboxSignature = signature;
        ...
        function editListing(slug) {
            ...
            updateAreaDropdown();
            updateCategoryDropdown();
            syncAmenitiesFromListing(listing);
            ...
            initAccordionEditorsForListing(listing);
        ...
        function saveListing(event) {
            ...
            applyFilterOptionCleanup();
            const savedOldSlug = editingSlug;
            const savedIsUpdate = isUpdate;
            listingFormOriginalData = null;
            closeModal(true);
            reconcileListingsGridAfterSave(listing, savedOldSlug, savedIsUpdate);
            showUnsavedChangesBadge();
        }
...
// Fallback: If critical functions fail to load, show error after 2 seconds
        ...
        function ensureGoogleMapsLoaded() {
            ...
            mapsLoadPromise = new Promise(function(resolve, reject) {
                window.__adminMapsReady = function() { resolve(); };
                var script = document.createElement('script');
                script.src = 'https://maps.googleapis.com/maps/api/js?key=' + GOOGLE_MAPS_API_KEY + '&libraries=places&callback=__adminMapsReady';
                script.async = true;
                script.defer = true;
                script.onerror = reject;
                document.head.appendChild(script);
        ...
        function applyPendingMapMarkers() {
            if (!map || !pendingMapListings) return;
            updateMapMarkersImpl(pendingMapListings);
            pendingMapListings = null;
        }
        ...
        function updateMapMarkersImpl(listings) {
            if (!map) return;
        ...
        function toggleMap() {
            ...
            if (mapVisible) {
                ...
                ensureGoogleMapsLoaded().then(function() {
                    initMap();
                    if (pendingMapListings) applyPendingMapMarkers();
                    setTimeout(function() {
